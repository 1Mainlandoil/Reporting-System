...
const DataTable = ({
  ...
  const stickyLeftByKey = columns.reduce(
    (acc, column) => {
      if (!stickySet.has(column.key)) return acc
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="grid gap-3 md:hidden">
          {rows.map((row, index) => (
            <div
              ...
              className={clsx(
                'rounded-xl border border-white/5 bg-white/5 p-3',
      ...
      {/* Desktop table */}
      {rows.length ? (
        <div className="hidden overflow-x-auto scroll-smooth rounded-xl border border-white/5 md:block">
          <table className={clsx(tableClassName, 'divide-y divide-white/5 text-sm')}>
                  ...
                  <th
                    ...
                    className={clsx(
                      ...
                      stickySet.has(column.key) &&
                        'sticky z-20 border-r border-white/5 bg-[#0d1220]',
                    )}
                    style={
                      stickySet.has(column.key)
                        ? { left: `${stickyLeftByKey[column.key]}px`, minWidth: `${stickyColumnWidths[column.key] ?? column.minWidth ?? 180}px`, width: column.width ? `${column.width}px` : undefined }
                        : { minWidth: column.minWidth ? `${column.minWidth}px` : undefined, width: column.width ? `${column.width}px` : undefined }
                    }
                  >
                    {column.header}
                  </th>
            ...
            <tbody className="divide-y divide-white/5 bg-[#0d1220]">
              {rows.map((row, index) => (
                <tr
                  key={getRowKey(row, index)}
                  onClick={() => onRowClick?.(row)}
                  ...
                  {columns.map((column) => (
                    <td
                      ...
                      className={clsx(
                        ...
                        stickySet.has(column.key) &&
                          'sticky z-10 border-r border-white/5 bg-[#0d1220]',
                      )}
                      style={
                        stickySet.has(column.key)
                          ? { left: `${stickyLeftByKey[column.key]}px`, minWidth: `${stickyColumnWidths[column.key] ?? column.minWidth ?? 180}px`, width: column.width ? `${column.width}px` : undefined }
